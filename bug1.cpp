...
int main() {
    ...
    for (int i = 0; i < REP; i++) {
        ...
        for(int j = 0; j < SIZE_OF_SAMPLES; j++) {
            // the "j" variable was entered as "i" originally
            // it is now changed to the correct variable, "j"
            std::cout << arr[j] << " ";
        }
        std::cout << std::endl;
    }
...
void UniqRandInt(int max, int n, int result[]) {
    ...
    while(cntr < n) {
        ...
        r = r % (max + 1);
        // originally, the condition in the if statement was wrong
        // the exclamation mark was necessary so the condition would say
        // "if the number is not is the array"
